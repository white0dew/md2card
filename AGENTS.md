...
针对本仓库的 Markdown 转图片任务，增加以下硬门禁：

7. 不得只通过页面预览判断任务完成。
8. 必须使用浏览器实际导出 PNG 文件。
9. 必须将左侧 Markdown 原文与下载下来的 PNG 逐项对照，确认不存在文字缺失、文字截断、段落丢失或分页后内容不连续。
10. 若未完成上述对照，不得声明“已完成”“已修复”“可交付”。
...
本仓库是一个将 Markdown 转成卡片图片的 Next.js App Router 应用。路由与全局样式位于 `app/`。编辑器工作台放在 `components/workbench/`，卡片主题组件放在 `components/cards/`。渲染、分页、导出等共享逻辑集中在 `lib/`，状态管理在 `stores/`，客户端水合辅助在 `hooks/`，主题样式在 `styles/`。静态资源放在 `public/`，测试放在 `tests/`，设计与计划文档放在 `docs/superpowers/`。

## Build, Test, and Development Commands
- `pnpm install`：按 `pnpm-lock.yaml` 安装依赖。
- `pnpm dev`：启动本地开发环境，使用 Turbopack。
- `pnpm build`：生成生产构建。
- `pnpm start`：以生产模式启动已构建应用。
...
使用 TypeScript 与函数式 React 组件。现有 TS/TSX 与配置文件普遍采用 2 空格缩进，保持这一风格即可。组件文件使用 `PascalCase`，如 `DefaultCard.tsx`；多词模块文件优先使用 `kebab-case`，如 `editor-pane.tsx`；工具与状态模块沿用现有命名，必要时优先与所在目录保持一致。复用逻辑尽量放入 `lib/`、`hooks/` 或 `stores/`，不要把实现细节堆进 `app/page.tsx`。

## Testing Guidelines
测试使用 Node 内置 `node:test` 与 `assert/strict`。新测试放在 `tests/` 下，命名遵循 `*.test.mjs`，并尽量与被测模块或行为一一对应。优先为配置、导出逻辑、分页工具和状态行为补回归测试；纯界面改动若无法直接做浏览器测试，至少补齐可脱离 UI 验证的逻辑断言。

## Commit & Pull Request Guidelines
当前工作区不含 `.git` 元数据，因此这里无法从历史提交中提炼既有规范。建议先采用简短、祈使式的 Conventional Commits，例如 `feat: add glass theme preset` 或 `fix: preserve markdown on refresh`。PR 说明应包含变更目的、验证步骤（如 `pnpm lint`、`pnpm test`、必要时 `pnpm build`），涉及卡片样式或布局调整时附上截图。

## Configuration Notes
修改本地开发源配置前先检查 `next.config.ts`，因为已有测试覆盖 `allowedDevOrigins`。不要提交 `.next/` 构建产物，也不要把 `tmp-downloads/` 里的临时文件带入提交。
